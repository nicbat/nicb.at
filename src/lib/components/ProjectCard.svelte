    ...
    <article class="">
      <div class="flex flex-row justify-between float-right">
        <!-- {#each project.tags as tag}
          {#if tag == "Publication"}
            <span
              class="bg-tag-primary/25 rounded-full p-2 text-xs font-bold uppercase text-tag-primary"
              >{tag}</span
            >
          {:else}
            <span
              class="bg-tag-secondary/25 rounded-full p-2 text-xs font-bold uppercase text-tag-secondary"
              >{tag}</span
            >
          {/if}
        {/each} -->
    ...
    <div class="links">
      {#each project.links as link}
        <a
          ...
          class="mr-4 last:mr-0">{link.text}</a
        >
      {/each}
    </div>
  </div>
